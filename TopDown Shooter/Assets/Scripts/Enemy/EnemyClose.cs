...
public class EnemyClose : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogWarning("Не назначены точки патрулирования для врага!");
        }
    }
    ...
    void ChasePlayer()
    {
        ...
        // Если видим игрока, обновляем позицию
        if (CanSeePlayer())
        {
            lastKnownPlayerPosition = player.position;
        ...
        {
            // Если не видим, идем к последней известной позиции
            SetDestination(lastKnownPlayerPosition);
            
            // Если достигли последней позиции, ищем
            if (!agent.pathPending && agent.remainingDistance <= agent.stoppingDistance)
            {
    ...
    {
        // Простой поиск - поворачиваемся и смотрим вокруг
    ...
    void AttackPlayer()
    {
        ...
        // Если игрок убежал, продолжаем преследование
    ...
    void PerformAttack()
    {
        Debug.Log("Враг атакует игрока! Урон: " + attackDamage);
        
        // Проверяем, все еще в радиусе атаки
